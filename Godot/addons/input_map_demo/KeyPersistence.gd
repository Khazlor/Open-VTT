...
var keymaps_path: String
var keymaps: Dictionary
var temp_keymaps: Dictionary


func _ready() -> void:
	keymaps_path = Globals.base_dir_path + "/keymaps.dat"
...
func apply_temp_keymap():
	for action in temp_keymaps:
		#replace event in action
		InputMap.action_erase_events(action)
		InputMap.action_add_event(action, temp_keymaps[action])
		#add to main dict
		keymaps[action] = temp_keymaps[action]
	#clear temp dict and save
	temp_keymaps.clear()
	save_keymap()
