...
#Desc:
#Resource for saving spell and ability data

#NOT USED ANYMORE - MOVED TO DATABASES - spell_database.gd

extends Resource
class_name Spell

@export var global = false #global spells are shared between campaings
@export var spell_name = ""
@export var character_classes: Array = []
@export var spell_level = 0
@export var spell_school: String = ""
@export var spell_icon: String #path to icon texture
@export var spell_attr_dict: Dictionary #dictionary for spell description and similar labels of spell card
@export var macro_dict: Dictionary #dictionary of macros linked by key to components

#spell card settings
@export var spell_card_components: Array = []
@export var spell_card_size: Vector2 = Vector2(200,300)
@export var spell_card_outline_width: float = 5
@export var spell_card_outline_color: Color = Color.BLACK
@export var spell_card_texture: String #path to texture
@export var spell_card_texture_offset: Vector2 = Vector2(0,0)
@export var spell_card_texture_scale: Vector2 = Vector2(1,1)

var spell_tree_item: TreeItem
		
#save spell resource to file
func save(resolve_conflict: bool = false):
	...
	if resolve_conflict:
		...
		if i != 0:
			spell_name = spell_name + "_" + str(i)
	if not DirAccess.dir_exists_absolute(path):
		DirAccess.make_dir_recursive_absolute(path)
	if not DirAccess.dir_exists_absolute(path):
		print("folder does not exist: " + path)
	var save = FileAccess.open(path + "/" + spell_name, FileAccess.WRITE)
	if save == null:
		print("file open error - aborting")
		print(error_string(FileAccess.get_open_error()))
		return
	store_spell_data(save)
	save.close()
	
func store_spell_data(save: FileAccess):
	save.store_var(spell_name)
	save.store_var(global)
	save.store_var(character_classes)
	save.store_var(spell_level)
	save.store_var(spell_school)
	save.store_var(spell_icon)
	
	save.store_var(spell_card_components)
	save.store_var(spell_card_size)
	save.store_var(spell_card_outline_width)
	save.store_var(spell_card_outline_color)
	save.store_var(spell_card_texture)
	save.store_var(spell_card_texture_offset)
	save.store_var(spell_card_texture_scale)
...
func get_path_to_save(include_name: bool = true):
	var base_path: String #spell folder path
	if global:
		base_path = Globals.base_dir_path + "/saves/Spells"
	else:
		base_path = Globals.base_dir_path + "/saves/Campaigns/" + Globals.campaign.campaign_name + "/Spells"
	...
	if include_name:
		...
		path = ""
	if spell_tree_item == null:
		print("spell tree_item is null !!!!")
		return base_path + "/" + path
	var item = spell_tree_item.get_parent()
	var root = spell_tree_item.get_tree().get_root()
